...
public class NovalnetAccountPageController extends AbstractSearchPageController
{
	...
	private static final String CUSTOMER_DATA_ATTR = "customerData";
	...
	private static final Logger LOG = Logger.getLogger(NovalnetAccountPageController.class);
	...
	protected void populateModelRegionAndCountry(Model model, String countryIsoCode)
	{
	...
	final String orderCode, final Model model, final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		try
		{
			OrderModel orderModel = novalnetOrderService.getOrder(orderCode);
			final OrderData orderDetails = orderFacade.getOrderDetailsForCode(orderCode);
			final List<NovalnetPaymentInfoModel> paymentInfo = novalnetDao.getNovalnetPaymentInfo(orderCode);
			if (paymentInfo != null && !paymentInfo.isEmpty())
			{

				NovalnetPaymentInfoData novalnetPaymentData = new NovalnetPaymentInfoData();
				novalnetPaymentData.setTransactionDetails(paymentInfo.get(0).getPaymentInfo());
				novalnetPaymentData.setOrderHistoryNotes(paymentInfo.get(0).getOrderHistoryNotes());
				model.addAttribute("novalnetPaymentInfo", novalnetPaymentData);
			}
			else
			{
				model.addAttribute("novalnetPaymentInfo", null);
			}
			...
			LOG.info("PaymentInfo: " + orderModel.getPaymentInfo());
			LOG.info("BillingAddress: " + address);
			if (address != null)
			{
				LOG.info("Billing country: " + address.getCountry());
			}
			...
			if (address != null)
			{
				...
				{
					countryData = addressDataUtil.getI18NFacade().getCountryForIsocode(address.getCountry().getIsocode());
				}
			}

			model.addAttribute(CUSTOMER_DATA_ATTR, customerFacade.getCurrentCustomer());
		...
		return NovalnetcheckoutaddonControllerConstants.Views.Pages.Account.AccountPage;
	}
	...
	public String profile(final Model model) throws CMSItemNotFoundException
	{
		...
		{
			model.addAttribute("title", NovalnetUtils.findTitleForCode(titles, customerData.getTitleCode()));
		}

		model.addAttribute(CUSTOMER_DATA_ATTR, customerData);
	...
	@GetMapping(value = "/edit-address/" + ADDRESS_CODE_PATH_VARIABLE_PATTERN)
	@RequireHardLogIn
	...
	@PostMapping(value = "/edit-address/" + ADDRESS_CODE_PATH_VARIABLE_PATTERN)
	@RequireHardLogIn
	...
	@PostMapping(value = "/select-suggested-address")
	...
	@GetMapping(value = "/set-default-address/" + ADDRESS_CODE_PATH_VARIABLE_PATTERN)
	@RequireHardLogIn
	...
	@PostMapping(value = "/set-default-payment-details")
	@RequireHardLogIn
	...
	@PostMapping(value = "/remove-payment-method")
	@RequireHardLogIn
	...
	@PostMapping(value = "/consents/give/{consentTemplateId}/{version}")
	@RequireHardLogIn
	...
	@PostMapping(value = "/consents/withdraw/{consentCode}")
	@RequireHardLogIn
	...
	@GetMapping(value = "/close-account")
	@RequireHardLogIn
